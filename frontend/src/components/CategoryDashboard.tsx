...
// Helper function to extract text color class from category colors string
function getTextColorClass(colorClasses: string): string {
  // Extract the text-* class from the color string
  const textColorMatch = colorClasses.match(/text-[\w-]+/);
  return textColorMatch ? textColorMatch[0] : "text-gray-500";
}
...
export function CategoryDashboard({
  ...
  const progress = dreams.reduce((acc, dream) => {
    if (!acc[dream.category]) {
      acc[dream.category] = { total: 0, completed: 0 };
    }
    acc[dream.category].total += 1;
    if (dream.completed) {
      acc[dream.category].completed += 1;
    }
    return acc;
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      {categories.map((cat) => {
        ...
        const { total, completed } = progress[cat] || {
          total: 0,
          completed: 0,
        };
        const percentage =
        ...
        const textColorClass = getTextColorClass(colorClasses);
        ...
        return (
          ...
          >
            ...
            <div className="space-y-2">
              <h3 className="font-semibold text-lg leading-tight opacity-90">
                {cat}
              </h3>
