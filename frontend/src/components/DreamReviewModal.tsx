...
import { X, Check, Trash2, Calendar, Tag, Type, AlertCircle } from "lucide-react";
...
export function DreamReviewModal({ dreams, onSave, onCancel }: DreamReviewModalProps) {
    const [editableDreams, setEditableDreams] = useState<DreamEntry[]>(dreams);
    const [showConfirmClose, setShowConfirmClose] = useState(false);
    const [isDirty, setIsDirty] = useState(false);

    const handleUpdate = (id: string, field: keyof DreamEntry, value: any) => {
        setIsDirty(true);
        setEditableDreams(prev => prev.map(d => 
            ...
            >
                <div className="p-6 border-b border-border flex justify-between items-center bg-secondary/20">
                    <div className="text-start">
                        ...
                        <p className="text-muted-foreground text-sm">Here is what I found. Verify and edit before we save them.</p>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                    {editableDreams.length === 0 ? (
                        ...
                        editableDreams.map((dream) => (
                            <div key={dream.id} className="grid grid-cols-12 gap-3 p-3 rounded-lg border border-border/50 bg-secondary/10 items-center hover:bg-secondary/20 transition-colors">
                                
                                {/* Title Input */}
                                <div className="col-span-12 md:col-span-5 relative">
                                    <Type className="w-4 h-4 absolute top-3 left-3 text-muted-foreground" />
                                    <input 
                                        type="text"
                                        value={dream.title}
                                        onChange={(e) => handleUpdate(dream.id, 'title', e.target.value)}
                                ...
                                <div className="col-span-6 md:col-span-3 relative">
                                    <Tag className="w-4 h-4 absolute top-3 left-3 text-muted-foreground" />
                                    <select
                                       value={dream.category}
                                       onChange={(e) => handleUpdate(dream.id, 'category', e.target.value as DreamCategory)}
                                       className={clsx(
                                ...
                                <div className="col-span-5 md:col-span-2 relative">
                                    <Calendar className="w-4 h-4 absolute top-3 left-3 text-muted-foreground" />
                                     <input 
                                        type="number"
                                        value={dream.suggested_target_year}
                                        onChange={(e) => handleUpdate(dream.id, 'suggested_target_year', parseInt(e.target.value) || new Date().getFullYear())}
                                        className="w-full bg-background/50 border border-border rounded-md py-2 pl-9 pr-3 text-sm focus:ring-1 focus:ring-primary outline-none"
                                        placeholder="Year"
                                    />
                                </div>
                ...
                <div className="p-4 border-t border-border bg-secondary/10 flex justify-end gap-3">
                    <button 
                        ...
                        className="px-4 py-2 rounded-lg text-muted-foreground hover:bg-secondary/50 transition-colors text-sm font-medium"
                    >
                        Cancel
                    </button>
                    <button 
                    ...
                    {showConfirmClose && (
                        ...
                        >
                            ...
                            >
                                ...
                                <h3 className="text-xl font-bold mb-2 text-start">Discard changes?</h3>
                                <p className="text-muted-foreground text-sm mb-6 text-start">You have unsaved changes. Are you sure you want to discard them?</p>
